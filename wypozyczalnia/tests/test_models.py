...
# //////////////// sprawdzanie czy działa on_delete=PROTECT /////////////////////
    def test_cant_delete_category_when_assigned_to_book(self):
        try:
            ...
            pass
        self.assertEqual(Category.objects.filter(name='category1').exists(), True)
    ...
    def test_delete_category_when_book_is_deleted(self):
        ...
        category.delete()
        self.assertEqual(Category.objects.filter(name='category1').exists(), False)
...
class OrderModelTest(TestCase):
    ...
    def setUp(self):
        ...
        Book.objects.create(title='book1',
                            ...
                            category=Category.objects.first(),
                            slug='book1', )
        OrderItem.objects.create(user=User.objects.first(), ordered_date=timezone.now(), item=Book.objects.first())
        Order.objects.create(user=User.objects.first(), ordered_date=timezone.now())
