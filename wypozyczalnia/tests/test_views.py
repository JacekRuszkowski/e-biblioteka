...
class TestHomeView(TestCase):
    ...
    def test_search_results(self):
        self.assertQuerysetEqual(Book.objects.filter(title__icontains='book1 0'), ['<Book: book1 0>'])
...
class TestCategoryAndAuthorView(TestCase):
    ...
    def test_category_view_correct_template(self):
        response = self.client.get(reverse('categories', kwargs={'slug': self.category.slug}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_author_view_correct_template(self):
        response = self.client.get(reverse('authors-books', args=[self.book.author]))
        self.assertEqual(response.status_code, 200)
...
class TestLoginLogic(TestCase):
    def setUp(self):
        ...
        testuser1 = User.objects.create_user(username='testuser1', email='testuser1@mail.com', password='Haslo123')
        testuser2 = User.objects.create_user(username='testuser2', email='testuser2@mail.com', password='Haslo123')
        admin = User.objects.create_superuser(username='superuser1', email='superuser1@mail.com', password='Haslo123')
        ...
        self.order1 = Order.objects.create(user=testuser1, ordered_date=timezone.now())
    ...
    def test_redirect_to_login_when_trying_to_borrow(self):
        response = self.client.get(reverse('borrow-book', args=[self.book1.slug]))
    ...
    def test_other_users_profile_preview_for_not_logged_user(self):
        ...
        response = self.client.get(reverse('profile-preview', args=[testuser2.username]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_cant_review_book_which_wasnt_borrowed(self):
        ...
        test_book = Book.objects.create(title='test_book',
                                        ...
                                        slug='test_book', )
        response = self.client.get(reverse('add-review', args=[test_book.slug]))
