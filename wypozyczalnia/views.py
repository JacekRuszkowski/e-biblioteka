...
def borrow_book(request, slug):
    ...
    if Order.objects.filter(user=request.user, ordered=False).exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            messages.warning(request, f'Możesz wypożyczyć tylko jeden egzemplarz danej książki.')
            return redirect('book-detail', slug=item.slug)
        else:
            if item.copies > 0:
                item.copies -= 1
...
def add_review(request, slug):
    ...
    if not OrderItem.objects.filter(user=request.user, item=book).exists():
        messages.warning(request, f'Nie możesz recenzować książki, która nie była przez Ciebie wypożyczona.')
    ...
    if request.method == "POST":
        ...
        if Review.objects.filter(review_author=request.user, review_book=book).exists():
            review = Review.objects.filter(review_author=request.user, review_book=book)[0]
